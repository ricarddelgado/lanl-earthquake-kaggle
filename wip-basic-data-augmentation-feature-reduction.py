...
train = pd.read_hdf('../tmp_results/Xv2_tr.hdf', 'data')
test = pd.read_hdf('../tmp_results/Xv2_test.hdf', 'data')
y = pd.read_hdf('../tmp_results/yv2_tr.hdf', 'data')  
...
lgbm_params = quick_hyperopt(train, y, 'lgbm', 150 ,diagnostic=False)
...
sub_1 = pd.read_csv('../input/sample_submission.csv')
...
sub_aug = pd.read_csv('../input/sample_submission.csv')
...
initial_imp = initial_imp[initial_imp['importance_score']!=-np.inf]
aug_imp = aug_imp[aug_imp['importance_score']!=-np.inf]
...
MIN_SCORE = np.percentile(feature_df.score_change.values.flatten(), 25)
...
sub_final.to_csv('sub_quartile_features_removed.csv', index=False)
