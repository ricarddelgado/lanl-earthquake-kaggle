    ...
    def get_next_batch(self, data, labels, train, step):
        ...
        if train:
            for i in range(self.batch_size):
                ...
                #feature = denoise_signal(high_pass_filter(feature, low_cutoff=10000, SAMPLE_RATE=fs), wavelet='haar', level=1)
            ...
            for i in range(self.batch_size):
                ...
                #val_batch = denoise_signal(high_pass_filter(val_batch, low_cutoff=10000, SAMPLE_RATE=fs), wavelet='haar', level=1)
...
class model(object):
    ...
    def create_cnn1d(self, x, bs, is_traning):
        ...
        #flat = tf.reshape(max_pool_3, (-1, 87*128))
    ...
    def create_lstmmodel(self,x, bs, is_training):
        ...
        with tf.variable_scope('MultiRNN', reuse=tf.AUTO_REUSE):
            ...
            print("Shape: " ,int(output.get_shape()[0]) - 1)
...
TARIN_VAL_SPLIT = 0.80
LOG_DIR = './folder_to_save_graph_3'
filewriter_path = LOG_DIR
checkpoint_path = ""
latest_checkpoint = "../input/2cnn2lstm/earthquake-epoch60.ckpt"
...
for seed in seeds:
    ...
    print("tTotal paramete4rs; ", total_parameters)
    ...
    with tf.Session(config = config) as sess:
        ...
        results_MSE.append(np.asarray(mse_validation).mean())
...
#     sess.run(tf.local_variables_initializer())
        ...
        for i, seg_id in enumerate(submission.index):
          #  print(i)
            ...
            #batch_test_submission = denoise_signal(high_pass_filter(batch_test_submission, low_cutoff=10000, SAMPLE_RATE=fs), wavelet='haar', level=1)
        ...
        submission.head()
        # Save
        submission.to_csv('submission_abs_gdg.csv')
        print(submission)
        print("FINISHED! :)")
